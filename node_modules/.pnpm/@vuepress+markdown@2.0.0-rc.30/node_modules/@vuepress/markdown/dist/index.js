...
const resolveLink = (link, { env, absolutePathPrependBase = false, aliasSupport = true }) => {
	if (link.startsWith("data:")) return link;
	let resolvedLink = decode(link);
	if (aliasSupport === false || aliasSupport === "@-prefix") {
		if (!(link.startsWith("/") || link.startsWith("./") || link.startsWith("../") || /[A-Za-z]+:\/\//.test(link))) {
			if (aliasSupport === false || !link.startsWith("@")) resolvedLink = `./${resolvedLink}`;
		}
	}
...
*/
const assetsPlugin = (md, { absolutePathPrependBase = false, aliasSupport = true } = {}) => {
	...
	const createHtmlRule = (rawHtmlRule) => (tokens, idx, options, env, self) => {
		tokens[idx].content = tokens[idx].content.replace(/(<(?:img|source|video|audio)\b.*?\bsrc=)(['"])(.*?)\2/gs, (_, prefix, quote, src) => `${prefix}${quote}${resolveLink(src.trim(), {
			env,
			absolutePathPrependBase,
			aliasSupport
		})}${quote}`).replace(/(<(?:img|source|video|audio)\b.*?\bsrcset=)(['"])(.*?)\2/gs, (_, prefix, quote, srcset) => `${prefix}${quote}${srcset.split(",").map((item) => item.trim().replace(/^([^ ]*?)([ \n].*)?$/, (__, url, descriptor = "") => `${resolveLink(url.trim(), {
			env,
			absolutePathPrependBase,
			aliasSupport
